...
public class DefaultCookieSerializer implements CookieSerializer {
            ...
            for (Cookie cookie : cookies) {
                if (this.cookieName.equals(cookie.getName())) {
                    ...
                    if ((this.jvmRoute != null)
                        ...
                        sessionId = sessionId.substring(0, sessionId.length()
    ...
    /**
     * Sets if a Cookie marked as secure should be used. The default is to use the value of
    ...
    /**
     * Sets if a Cookie marked as HTTP Only should be used. The default is true in Servlet 3+ environments, else false.
     * 
    ...
    /**
     * Sets the path of the Cookie. The default is to use the context path from the {@link HttpServletRequest}.
     * 
     * @param cookiePath
    ...
    /**
     * Sets the maxAge property of the Cookie. The default is -1 which signals to delete the cookie when the browser is
     * closed.
     * 
     * @param cookieMaxAge
    ...
    /**
     * Sets an explicit Domain Name. This allow the domain of "example.com" to be used when the request comes from
     * www.example.com. This allows for sharing the cookie across subdomains. The default is to use the current domain.
     * 
     * @param domainName
     ...
     * {@link HttpServletRequest#getServerName()}. The pattern should provide a single grouping that defines what the
     * value is that should be matched. User's should be careful not to output malicious characters like new lines to
     * prevent from things like <a href= "https://www.owasp.org/index.php/HTTP_Response_Splitting">HTTP Response
     * Splitting</a>.
     * </p>
     * 
     * <p>
     * If the pattern does not match, then no domain will be set. This is useful to ensure the domain is not set during
     * development when localhost might be used.
     * </p>
     * <p>
     * An example value might be "^.+?\\.(\\w+\\.[a-z]+)$". For the given input, it would provide the following explicit
     * domain (null means no domain name is set):
     * </p>
     * 
     * <ul>
     ...
     * no performance benefit. However, this can help with tracing logs of a particular user. This will ensure that the
     * value of the cookie is formatted as
     * </p>
     * <code>
     * sessionId + "." jvmRoute
     * </code>
     * <p>
     * To use set a custom route on each JVM instance and setup a frontend proxy to forward all requests to the JVM
     * based on the route.
     * </p>
     * 
     * @param jvmRoute
